...
//Si viene un id, se carga la pregunta 
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    $idPregunta = (int)$_GET['id'];
    $datosCargados = leerPreguntaPorId($idPregunta); 
...
//Si se envía el formulario de editar, se guardan los datos
if (isset($_GET['guardar_pregunta'])) {
    ...
    if (!$informacionCompleta) {
        ...
        if ($idPreguntaRecibida !== null) {
            $mensaje = "Pregunta #$idPreguntaRecibida editada con éxito.";
        } else {
            $mensaje = "Nueva pregunta guardada con éxito.";
        }
...
if (!$esAdmin) {
    ...
    if (!empty($preguntasExistentes)) {
        echo '<h3 class="mt-5 mb-3">Preguntas Existentes (Haga clic para editar)</h3>';
        echo '<div style="max-height: 600px; overflow-y: auto; border: 1px solid #dee2e6; border-radius: 8px; background-color: #fff; box-shadow: 0 2px 5px rgba(0,0,0,0.05);">'; 
        echo '<ul class="list-group list-group-flush">';
        foreach ($preguntasExistentes as $index => $pregunta) {
            echo '<li class="list-group-item d-flex justify-content-between align-items-center py-3">';
            echo '<div style="flex-grow: 1; margin-right: 15px; font-size: 1.05rem;">';
            echo '<strong>#' . $index . ':</strong> ' . htmlspecialchars($pregunta[0]);
            echo '</div>';
            echo '<a href="editar.php?id=' . $index . '" class="btn btn-sm btn-outline-primary px-3" style="flex-shrink: 0; border-radius: 20px;">Editar</a>';
            echo '</li>';
        }
        echo '</ul>';
